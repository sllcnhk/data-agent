...
import { Button, Drawer, Empty, Input, Spin, Tag, Tooltip, message as antMessage } from 'antd';
import {
  CloseOutlined,
  RobotOutlined,
  SendOutlined,
...
export interface DataCenterCopilotProps {
  open: boolean;
  onClose: () => void;
...
const DataCenterCopilot: React.FC<DataCenterCopilotProps> = ({
  ...
  // Show greeting when drawer opens for the first time
  React.useEffect(() => {
  ...
  const appendToLastAssistant = useCallback((text: string) => {
    setMessages((prev) =>
      prev.map((m) => (m.id === currentAsstIdRef.current ? { ...m, content: m.content + text } : m))
    );
  }, []);
        ...
        for (const line of chunk.split('\n')) {
          ...
          try {
            ...
            if (data.type === 'content') {
              ...
              // Detect if AI called PUT /reports/{id}/spec
              const resultStr =
