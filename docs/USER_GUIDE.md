...
首次启动多用户模式前，需运行初始化脚本，将 4 个预置角色和 21 个权限写入数据库：
...
# 响应：21 条权限定义列表，每条含 id / resource / action / description
```
...
| 千万行级数据按日期分多个 Excel 文件交付 | 数据导出页 → 导出 Modal 切换「按日期分块（多文件）」→ 配置日期范围与单块天数（见 13.7 节）|
...
GET    /api/v1/permissions                       # 全量权限定义列表（共 21 条，含 reports:*/schedules:*）
...
| 当前 Sheet | 正在处理的工作表名 |
| 错误信息 | 失败时显示批次号和错误原因；遇 ClickHouse 并发上限自动退避重试时，也会实时追加一条 `level: "warning"` 记录（说明第几批、第几次重试），据此可区分「正在退避等待」与「真的卡死」|
...
| 批大小 | 每批 **5000** 行（v2.17.1 由 1000 提升；前端页面写死此值、UI 不可调）。程序化调用可在 `ExecuteImportRequest.batch_size` 中指定，范围 100–50000。同一文件的 INSERT 次数因此降为原来的 1/5，撞上 `Code: 202` 并发上限的机会同比下降；代价是单批 body 变大（长文本列可达 ~20 MB），故批插入超时同时放宽到 180 秒（见 12.5）|
...
| 并发上限重试 | 分批插入遇 `Code: 202 TOO_MANY_SIMULTANEOUS_QUERIES` 时自动固定间隔退避重试（默认 5 次 × 15 秒，见 12.5）。**仅**该错误码重试（失败批次 0 行入库，重发安全）；超时 / 连接错误等其他异常立即终止任务，不重试（服务端可能已写入，重发会产生重复行）|

### 12.5 环境变量

```bash
# 分批插入遇 ClickHouse 并发上限（Code: 202 TOO_MANY_SIMULTANEOUS_QUERIES）时的退避重试
IMPORT_TOO_MANY_RETRY_MAX=5               # 最大重试次数，默认 5；0=禁用，遇 202 立即失败
IMPORT_TOO_MANY_RETRY_BACKOFF=15          # 每次重试前等待秒数（固定值，不递增），默认 15

# 批插入的 HTTP 超时（秒）
IMPORT_INSERT_TIMEOUT=180                 # 默认 180；只作用于批插入，schema 查询（库/表列表）仍固定 60s
```

- 每次运行任务时读取，改完 `.env` 重启生效
- 单批最长阻塞 = `RETRY_MAX × RETRY_BACKOFF`（默认 75 秒）；等待期间每秒检查一次取消状态，点「取消」最多 1 秒生效
- 每次重试以 `level: "warning"` 写入 `import_jobs.errors` 并立即落库，前端进度页的「错误」列可见：
  **橙色徽标 = 重试次数（任务仍在继续，不代表数据丢失）**，红色徽标 = 真正失败的批次数，两者分开计数。
  历史数据中没有 `level` 字段的条目一律按真失败（红色）处理
- 命名与导出侧的 `EXPORT_TOO_MANY_RETRY_MAX` / `EXPORT_TOO_MANY_RETRY_BACKOFF`（见 13.8）对称，但两侧默认值不同（导出退避 10 秒，导入 15 秒），互不影响

> **`IMPORT_INSERT_TIMEOUT` 为什么默认 180 而不是 60**（v2.17.1，2026-09-08）：本次把批大小
> 从 1000 提到 5000 后，含长文本列的单批 body 可达约 20 MB（如 `call_record_imported` 的
> `Call Record Text Detail Masked` 对话详情）。THAI 环境实测 19.62 MB 单批插入**典型 6.3 秒、
> 最差 19.8 秒**（跨公网抖动，呈双峰），已占原 60 秒超时的 33%。而超时抛出的 `TimeoutError`
> 是**唯一不能重试**的错误（服务端可能已写入成功，重发会造成重复行），一次假超时就会让整个
> 任务失败且无法自愈。放宽到 180 秒是为了避免批大小提升把「可自愈的 202」变成「不可自愈的
> 超时」。跨境链路更慢或单行文本更长时可继续上调。
