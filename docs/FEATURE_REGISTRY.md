...
| RBAC 初始化脚本 | `backend/scripts/init_rbac.py` | 幂等脚本，写入 4 角色（viewer/analyst/admin/superadmin）+ 21 权限（含 data:import/export、reports:read/create/delete、schedules:read/write/admin）；admin 无 users:* 权限 |
...
superadmin 专属功能（可动态授予其他角色）：执行任意 SQL 查询并将结果异步导出为 Excel / CSV / CSV ZIP 文件，支持多 Sheet 自动分割、大整数安全转换、实时进度轮询和任务取消/下载/删除。
...
#### 按日期分块导出（v2.13，2026-05-07）

针对千万级月明细数据导出场景，新增按日期分块导出能力：单 SQL → N 个日期窗口 → N 个 xlsx 文件，块内继续保持 1M 行/Sheet 自动拆分。

| 功能 | 文件 | 说明 |
|------|------|------|
| 模型扩展 | `backend/models/export_job.py` | 新增 `export_mode`(single\|date_chunked) / `chunk_config`(JSONB) / `output_files`(JSONB)；`to_dict()` 新增三字段；非破坏性变更，旧任务全 `single` 模式 |
| 迁移脚本 | `backend/scripts/migrate_export_chunked.py` | ALTER TABLE 加三列（IF NOT EXISTS），存量回填 `export_mode='single'` |
| 切分纯逻辑模块 | `backend/services/data_export_chunker.py` | 无 IO 工具：`split_date_range`（闭区间，最后块自适应）/`inject_date_filter`（占位符 + 包装双模式）/`build_chunk_filename`/`validate_chunk_config`；列名严格白名单防 SQL 注入 |
| 服务层重构 | `backend/services/data_export_service.py` | 抽出 `_run_single_export`（单文件流式 + Code 160 自动回退）→ 单文件路径与分块路径共用；新增 `_run_chunked_export_sync`：串行调度 N 块，每块更新 `output_files` 清单与累计进度 |
| API 扩展 | `backend/api/data_export.py` | `ExecuteExportRequest.chunk_config?` 可选字段；`POST /execute` 提早校验 chunk_config 失败抛 400；`GET /jobs/{id}/download?file_index=N` 支持按块下载；`DELETE /jobs/{id}` 递归删除分块目录 |
| 前端 API 客户端 | `frontend/src/services/dataExportApi.ts` | 新增 `ChunkConfig` / `ExportFileEntry` / `ExportMode` 类型；`downloadFile(jobId, fileIndex?)` 透传 `?file_index=N` |
| 前端页面 | `frontend/src/pages/DataExport.tsx` | Modal 新增导出模式 Radio（单文件 / 按日期分块）；分块表单（RangePicker + chunk_days InputNumber + date_column Input）；任务列表行可展开显示 `output_files` 子表，每子文件独立下载按钮 |
| 测试 — 单元 | `test_data_export_chunker.py` | 51 用例（A 切分 16 + B 注入 12 + C 文件名 7 + D 校验 16） |
| 测试 — 集成 | `test_data_export_chunked.py` | 17 用例（E API 4 + F 串行 2 + G 失败 1 + H 取消 2 + I 下载 4 + J 删除 1 + L 注入路径 2 + M Sheet 拆分 1） |
| 测试 — e2e | `test_data_export_chunked_e2e.py` | 9 用例（N 前端契约 3 + O 状态返回 2 + P 全流程 2 + Q 混合列表 1 + R 占位符替换 1） |

**两种 SQL 注入路径**：
- **占位符模式（推荐）**：用户 SQL 含 `{{date_start}}` / `{{date_end}}` → 字面量字符串替换。谓词由 ClickHouse 直接下推，性能最佳。
- **包装模式（兜底）**：用户 SQL 不含占位符 → 包装为 `SELECT * FROM (user_sql) AS _chunk_q WHERE _chunk_q.{date_column} >= '...' AND <= '...'`。简单但复杂查询（GROUP BY/JOIN）无法谓词下推。

**取消语义**：当前块写到一半被取消 → 已写数据保留（`output_files[i].status="cancelled"`），后续块跳过；整体 Job → cancelled。便于用户保留部分成果。
...
| 功能 | 文件 | 说明 |
|------|------|------|
| query_id + KILL QUERY | `backend/services/export_clients/clickhouse.py` | `stream_batches` / `stream_batches_keyset` / `stream_raw` 为每个 ClickHouse HTTP 查询注入唯一 `query_id`；流式异常时 best-effort `KILL QUERY WHERE query_id=... SYNC`，避免 HTTP 连接断开后服务端旧查询继续占用并发。 |
| env 级导出并发闸门 | `backend/services/data_export_service.py` | `_get_export_query_semaphore(env)` 默认同一 ClickHouse env 仅 1 个导出查询并发；可通过 `EXPORT_MAX_CONCURRENT_QUERIES_PER_ENV` 调整。 |
| TOO_MANY 退避重试 | `backend/services/export_clients/clickhouse.py` + `data_export_service.py` | 新增 `is_ch_too_many_queries_error`；遇到 Code 202 自动退避重试（`EXPORT_TOO_MANY_RETRY_MAX` / `EXPORT_TOO_MANY_RETRY_BACKOFF`），不再立即失败。 |
| CSV / CSV ZIP 极速导出 | `data_export_service.py` + `data_export.py` + `DataExport.tsx` | `ExecuteExportRequest.output_format ∈ {xlsx,csv,csv_zip}`；CSV 路径使用 ClickHouse `FORMAT CSVWithNames` 原始字节流直写，写入 UTF-8 BOM，适合 Excel 打开中文。分块 `csv_zip` 会先生成 CSV 子文件，完成后打包 ZIP。 |
| XLSX CSV staging | `data_export_service.py` | `xlsx_engine ∈ {auto,direct,csv_staging}`；`auto` 下分块/大数据默认先落 CSV 临时文件，再用 `csv.reader(newline=\"\")` 本地转换 XLSX，正确处理引号内逗号/换行，减少远端查询连接占用时间。 |
| 预分裂时间窗口 | `data_export_service.py` + `ChunkConfigSchema.pre_split_hours` | 启用 hour/minute 再细分时，默认按 6 小时预拆；用户可填 1~24 小时。避免先执行整天大块约 5 分钟后断流再分裂的浪费。 |
| 前端 UI | `frontend/src/pages/DataExport.tsx` | 导出 Modal 新增输出格式、XLSX 写入策略、预分裂窗口；分块 CSV ZIP 完成后支持整包下载。 |
| 测试 | `test_data_export.py::TestCsvStagingToXlsx` + 既有导出 E2E/keyset/chunker 回归 | 新增 CSV→XLSX 中文、逗号、引号内换行、大整数文本保真测试；回归预览、E2E、keyset、ts placeholder。 |

#### 数据导出可靠性增强（v2.14，2026-05-11）

针对**跨境/云上长查询 + 千万级行 + 重型 SQL**导出失败场景,四项独立改进 + 修两个隐性 bug。所有字段 opt-in,默认行为与 v2.13 完全一致。

| 任务 | 文件 | 说明 |
|------|------|------|
| **Task C** Windows TCP keepalive 真正生效 | `backend/services/export_clients/clickhouse.py` | 新增 `_WinKeepAliveHTTPConnection/Pool` 子类化 urllib3;`win32` 平台 `init_poolmanager()` 注册 win pool 类,`connect()` 后用 `sock.ioctl(SIO_KEEPALIVE_VALS, (1, idle_ms, intvl_ms))` 设置心跳(毫秒级)。修 v2.13 在 Win 上只挂 `SO_KEEPALIVE`(注册表默认 2h 间隔等同无效)的隐性 bug。 |
| **Task A** 小时/分钟级再细分(opt-in) | `backend/services/data_export_chunker.py` + `data_export_service.py` | `subdivide_range(start, end, min_unit)` 通用化对半分裂(支持 `Union[date, datetime]`);`min_subdivide_unit ∈ {day, hour, minute}` 控制最小窗口,day=老行为,hour/minute 允许 1 天块失败后下钻;`inject_date_filter` + `build_chunk_filename` 支持 datetime 端点 + 生成 `YYYYMMDDTHHMMSS` 文件名。 |
| **Task B** 键集分页替代 LIMIT/OFFSET(opt-in) | `backend/services/export_clients/clickhouse.py` + `data_export_service.py` | 新增 `stream_batches_keyset(sql, cursor_column, batch_size)`:首窗口 `ORDER BY cursor LIMIT N`,后续 `WHERE cursor > {cursor_val:String}` 用 CH HTTP 参数化推进;`_run_single_export(cursor_column=)` fallback 时优先 keyset。修 v2.13 的 LIMIT/OFFSET 无 ORDER BY 在并行扫描下窗口可能重叠/漏行的隐性正确性 bug。 |
| **Task D** 失败先重试一次再分裂 | `backend/services/data_export_service.py` | `_run_chunked_export_sync` 异常分支前置 retry 阶段:`_retry_count < MAX_INPLACE_RETRY(env, 默认 1)` 时 5s 退避(分秒 cancel 检查)+ 原位重试,失败再进入分裂。`output_files[i]` 加 `_retry_count` 内部字段。 |
| Bug 修 1 — 异常链探测 | `backend/services/export_clients/clickhouse.py` | `is_transient_stream_error(exc)` 沿 `__cause__/__context__` 递归探测,识别 `RuntimeError("分批模式预扫描行数失败:...") from ChunkedEncodingError` 这类包装层下的瞬时错误。修 v2.13 包装异常吞掉 transient 指纹 → 外层 retry 不触发的 bug。 |
| Bug 修 2 — keyset 单调性 | `backend/services/export_clients/clickhouse.py` | cursor 单调性检查从严格 `>` 改为只防死循环(`==`)。修字典序数字 cursor `"50"` > `"100"` 误判为非单调的 bug(因为 cursor 通过 TSV 字符串返回)。 |
| API 扩展 | `backend/api/data_export.py` | `ChunkConfigSchema` 加 `min_subdivide_unit: Optional[Literal["day","hour","minute"]] = "day"` + `cursor_column: Optional[str] = None`;**无新端点**,沿用现有 `data:export` 权限。 |
| 前端类型 + UI | `frontend/src/services/dataExportApi.ts` + `pages/DataExport.tsx` | `ChunkConfig` 加两字段;Modal 加 Select(最小再细分粒度)+ Input(游标列名),折叠在高级区;`ChunkFileList` 渲染含 `T` 的 datetime ISO 时把 `T` 替换为空格易读。 |
| 测试 — 单测 | `test_data_export_keepalive.py` 11 + `test_data_export_keyset.py` 9 | K1-K6(平台分支 + ioctl 参数 + 降级)+ G1-G6(keyset 首/后续窗口 + 单调 + 列不存在 + 死循环)+ H1-H2(选路) + I1(集成) |
| 测试 — 集成 | `test_data_export_chunker.py` F+B+C+D 段补 22 + `test_data_export_chunked.py` KN/L 段 5 | F1-F10 `subdivide_range` + B13-B15 datetime inject + C8-C9 datetime 文件名 + D17-D23 新字段 + KN1-KN2 sub-day + L1-L3 Task D |
| 测试 — E2E | `test_data_export_v214_e2e.py` 20 | M1-M3 API round-trip + N1-N3 边界 + O1-O4 RBAC 静态验证(无新端点/菜单)+ P1 组合(hour+keyset)+ Q1 cancel race + R1 keyset 多窗口 + S1-S2 datetime 文件下载 + T1-T3 异常链 + U1-U2 Windows 文件名安全 |

**RBAC 验证(本次重点)**:
- 全部字段在请求 body 内,沿用 `data:export` 权限保护 8 个现有端点(`O3` 测试静态检查)
- 前端菜单未新增子项(`O2` 测试)
- `init_rbac.py` 权限矩阵无需变更(`O4` 测试)
- 默认仅 `superadmin` 角色拥有 `data:export`(可通过角色管理 API 动态授予其他角色)

**总测试新增数**:Task C 11 + Task A 24 + Task B 9 + Task D 3 + v2.14 e2e 20 = **67 测试**;全量回归 332 通过零失败。

**DB 迁移**:**无**(`min_subdivide_unit` / `cursor_column` 都是 `chunk_config` JSONB 字段内扩展,SQLAlchemy 无需 ALTER TABLE)。

**关键模式**(供 LLM/未来维护者参考):
- 占位符 SQL 用户须自行加单引号:`WHERE ts >= '{{date_start}}'`,sub-day 模式下替换为 `'2025-04-01 00:00:00'` 仍是有效字面量
- DateTime 列 + sub-day → 精确过滤;Date 列 + sub-day → CH 自动截断到 Date,子块返回相同行 → 浪费递归(opt-in 用户责任)
- keyset 不适用于 `GROUP BY / DISTINCT` 等聚合 SQL(cursor 推进语义无效);运行时遇 cursor 列重复值 → fast-fail 死循环错误
- **v2.14.1 cursor_column 校验放宽**:接受字母/数字/下划线 + **空格** + **中文** 起首(字母/下划线/中文);用户可填裸列名(`Call ID`)或带反引号(`` `Call ID` ``,自动 strip);拼接 SQL 时统一反引号包裹 `` ORDER BY `{cursor_column}` `` 适配含空格/中文的别名;`col_names.index()` 用裸字符串查找(TSV header 列名不含反引号)。新增测试 D24-D29(chunker) + G7-G8(client)。子查询 `_ks_q` 暴露的是用户 SELECT 的**别名**,所以 cursor_column 必填别名(原表 `table.col` 在外层不可见)。
- **v2.14.4 ts 半开区间占位符**:新增 `{{ts_start}}` / `{{ts_end}}` 一对占位符,**始终输出 `'YYYY-MM-DD HH:MM:SS'` 字面量,且 `ts_end` 等于「下一窗口起点」(exclusive end)**:
  - day 模式:`ts_start='YYYY-MM-DD 00:00:00'`,`ts_end='YYYY-MM-(DD+1) 00:00:00'`
  ...
  - 用户 SQL 统一写法:`WHERE col >= parseDateTimeBestEffort('{{ts_start}}') AND col < parseDateTimeBestEffort('{{ts_end}}')`,**day 与 sub-day 完美兼容,无重叠无遗漏,DateTime64 亚秒精度安全**。
  - 经典 `{{date_start}}` / `{{date_end}}` 闭区间占位符**保留不变**;两套并存可在同一 SQL 各自替换(允许混用,但通常只用一套)。新增测试 V1-V8(chunker)+ KN3(集成 sub-day 衔接)。
  - 解决场景:用户 SQL 用 `addDays(toDate('{{date_end}}'), 1)` 时 sub-day 失效(toDate 截断);用 `+ INTERVAL 1 SECOND` 时 sub-day 拆 12h+12h 实际取 36 小时;ts 占位符两个问题都不存在。
...
### 动态报表"Failed to fetch"根因修复（已完成，2026-04-16，v2.11.1）

**功能描述**：修复动态报表在 iframe 预览时，图表数据区域显示"查询失败: Failed to fetch"的问题。三大根因全部修复，无 DB 迁移，无新权限。

| 子功能 | 状态 | 说明 |
|--------|------|------|
| 根因 A：API_BASE IIFE 修复 | ✅ | `report_builder_service.py`：HTML API_BASE 改为 `(function(){ var h="{url}"; return h||(window.location.origin+'/api/v1'); })()`；`report_service.py:_api_base_url()`：无 PUBLIC_HOST 时返回 `""` 而非 `"http://localhost:8000/api/v1"` |
| 根因 B：connection_env 前缀剥离 | ✅ | `backend/api/reports.py:_get_or_init_ch_client()`：自动剥离 `"clickhouse-"` 前缀（兼容 LLM 误传 `"clickhouse-sg"` 格式）|
| 根因 C：binds list→dict 容错 | ✅ | `backend/services/report_params_service.py`：新增 `_normalize_binds(binds)` 辅助函数；list `["date_start","date_end"]` 自动转为 dict `{"start":"date_start","end":"date_end"}`；两个消费函数均已改用 |
| 根因 D：skill 文件格式警告 | ✅ | `.claude/skills/project/clickhouse-analyst.md`：零-B 节后补充 connection_env 和 binds 格式⚠️警告 |
| RBAC 范围 | ✅ | 无新菜单/路由/权限键；token-only 端点（/data、/html）为有意设计（iframe/分享场景） |
| 无 DB 迁移 | ✅ | 纯代码层 Bug 修复，无新表、新列、新权限记录 |
| 测试套件 | ✅ | `test_report_fetch_e2e.py`（60 tests，H-O 段：FastAPI TestClient / SQL 参数 / Jinja2 / HTML Builder / 权限矩阵 / E2E / 边界 / CH 客户端）+ `test_dynamic_report_fetch.py`（G1-G4 新增 24 tests，共 60 tests）|

**核心文件**：`backend/api/reports.py` / `backend/services/report_service.py` / `backend/services/report_builder_service.py` / `backend/services/report_params_service.py` / `.claude/skills/project/clickhouse-analyst.md`
...
| `test_dynamic_report_fetch.py` | 动态报表参数化 + "Failed to fetch" 修复验证（A-G 段，G1-G4 新增：API_BASE / connection_env / binds / skill 四层；共 60 tests）| **60** |
| `test_report_fetch_e2e.py` | 动态报表 E2E（H-O 段：FastAPI TestClient / SQL 参数 / Jinja2 / HTML Builder / 权限矩阵 / E2E / 边界 / CH 客户端）| **60** |
| **合计** | | **~1908** |
