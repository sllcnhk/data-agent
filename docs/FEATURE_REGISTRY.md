...
> **最后更新**：2026-04-14（**AI Pilot 实时助手 v2.9**：`DataCenterCopilotContent` 具名导出 + `ModelSelectorMini` 紧凑模型选择器 + `ReportPreviewModal` pilotContext prop + `_inject_pilot_button(doc_type)` HTML 注入路由修复 + `POST /reports/{id}/copilot` + `POST /scheduled-reports/{id}/copilot` + autoPilot URL 参数处理 + 3 处 Bug 修复（模型切换/HTML路由/Documents缺失autoPilot）；无 DB 迁移；**数据管理中心 + 定时推送任务 v2.5**：`migrate_datacenter_v1.py` DB 迁移；`ScheduledReport`/`ScheduleRunLog`/`NotificationLog` ORM 模型；`scheduler_service.py` APScheduler 封装；`notify_service.py` 4 渠道通知；9 端点 `/scheduled-reports/*` REST API；`DataCenterLayout` + 3 子页面前端；3 个 Co-pilot 技能；`schedules:read/write/admin` RBAC 权限；K1-K8 单元测试 + L1-L6 E2E 测试；`reports` 表新增 `doc_type/scheduled_report_id/version_seq`；`GET /reports` 增 `doc_type` 过滤；`PUT /reports/{id}/spec` 重生成端点；报告分享 API；307重定向 Bug 修复；v2.4 2026-04-13：**多图表 HTML 报告生成**；v2.4 2026-04-08：**Skill 用户使用权限隔离 T1–T6**；其余历次变更见下方记录）
...
| 功能 | 文件 | 说明 |
|------|------|------|
| update-report.md | `.claude/skills/project/update-report.md` | 触发词：更新报表/修改图表/调整报告等；引导 AI 使用 `PUT /reports/{id}/spec` 更新现有报告 |
