...
> **最后更新**：2026-04-15（**v2.10 报表增强**：**无 DB 迁移**，图表控件 ⋮ 菜单 + `GET /spec-meta` + `PUT /charts/{chart_id}` + `ReportToolMCPServer` + `ReportViewerPage` + Pilot 一对一绑定均为纯代码层变更，复用已有 `reports` 表的 `charts` JSONB 列和 `version_seq` 整数列；**v2.9 2026-04-14**：AI Pilot 实时助手 + 对话报表手动固定 Pin：无 DB 迁移；2026-04-13：数据管理中心 v1；其余历次变更见下方记录表）
