...
> **最后更新**：2026-05-12(**v2.14.4 ts 半开区间占位符**:**无 DB 迁移**,新增 `{{ts_start}}` / `{{ts_end}}` 一对占位符,始终输出 datetime 字面量 + 半开区间语义,day 模式与 sub-day 拆分完美兼容,无重叠无遗漏;经典 `{{date_start}}` / `{{date_end}}` 占位符保留向后兼容;**v2.14.3 Excel 引擎切换 openpyxl→xlsxwriter**:**无 DB 迁移**,`_run_single_export` 切换到 xlsxwriter `constant_memory` 模式,性能 3-5x 提升,内存峰值更低;输出 100% 兼容(大整数文本/公式样字符串/None 处理一致);341/341 回归零失败;**v2.14.2 NULL cursor 错误信息升级**:**无 DB 迁移**,客户端 keyset 末行 NULL 报错时给出 4 个可执行修复建议;**v2.14.1 cursor_column 校验放宽**:**无 DB 迁移**,纯代码层 — 校验正则从 `_IDENT_RE` 替换为 `_CURSOR_IDENT_RE` 允许空格/中文,自动剥反引号,拼接 SQL 时统一反引号包裹;支持用户 SELECT 别名 `Call ID` / `订单_id` 等直接作为 keyset 游标);**v2.14 数据导出可靠性增强**：**无 DB 迁移**,Task C/A/B/D 四项独立改进均为代码层 + 配置字段扩展。`chunk_config` JSONB 新增 `min_subdivide_unit` / `cursor_column` 字段;`output_files[].`新增 `_depth` / `_retry_count` 内部字段;均沿用已有 `export_jobs.chunk_config` JSONB 列(v2.13 已加入,见下方记录)和 `output_files` JSONB 列,无 ALTER TABLE。**v2.13 按日期分块导出**:见下方记录(2026-05-07);**v2.11.1 动态报表 "Failed to fetch" 根因修复**：**无 DB 迁移**，纯代码层 Bug 修复（API_BASE IIFE + connection_env 前缀剥离 + binds list→dict）；**v2.11 参数化动态报表**：**无 DB 迁移**，`GET /reports/{id}/data` 端点 + `report_params_service.py` + `_autoDetectFields` + series 模板修复均为纯代码层变更，使用已有 `reports.charts` JSONB（含 SQL 模板/x_field 等）和 `reports.filters` JSONB（含 binds 字段），无新表、新列；**v2.10 报表增强**：**无 DB 迁移**，图表控件 ⋮ 菜单 + `GET /spec-meta` + `PUT /charts/{chart_id}` + `ReportToolMCPServer` + `ReportViewerPage` + Pilot 一对一绑定均为纯代码层变更，复用已有 `reports` 表的 `charts` JSONB 列和 `version_seq` 整数列；**v2.9 2026-04-14**：AI Pilot 实时助手 + 对话报表手动固定 Pin：无 DB 迁移；2026-04-13：数据管理中心 v1；其余历次变更见下方记录表）
...
| AI Pilot 实时助手（v2.9） | 2026-04-14 | **无迁移脚本**。纯代码层变更，**复用现有 `conversations` 表 + `current_model` 字段**。新增 `POST /api/v1/reports/{id}/copilot` 和 `POST /api/v1/scheduled-reports/{id}/copilot` 端点，在 `conversations` 表中以系统提示注入报表/推送任务 spec 上下文的方式创建 Co-pilot 对话。三个前端入口（HTML 报告内 FAB + ReportPreviewModal 内嵌面板 + DataCenter 列表 AI 按钮）均通过这两个端点获取 `conversation_id` 后接入现有 SSE 流式对话。所需 RBAC 权限（`reports:read` + `schedules:read`）已在 `migrate_reports_permissions.py` 及 `migrate_datacenter_v1.py` 中种子，无需补录。 | ✅ 无需执行 |
| 报表增强 v2.10（图表控件 / MCP 工具 / 分屏查看） | 2026-04-15 | **无迁移脚本**。全部为代码层变更，无新增 DB 列或表：① `_inject_chart_controls()` 向 HTML 注入图表 ⋮ 菜单（Force Refresh / Fullscreen / View Query / Download），复用 `reports.refresh_token` 和 `reports.charts` 已有字段；② `GET /reports/{id}/spec-meta` 新端点，无状态读取，不写 DB；③ `PUT /reports/{id}/charts/{chart_id}` 局部 merge 更新，复用已有 `reports.charts` JSONB 列和 `reports.version_seq` 整数列（均在 `migrate_datacenter_v1.py` 中已添加）；④ `ReportToolMCPServer`（3 MCP 工具）纯代码，不接触 DB；⑤ `ReportViewerPage`（`/report-view`）纯前端，不接触 DB；⑥ Pilot 一对一 upsert 绑定，复用已有 `conversations.extra_metadata` JSONB 中 `context_type/context_id` 字段。所需 RBAC 权限（`reports:create` / `reports:read`）均在 `migrate_reports_permissions.py` 中已种子，无需补录。 | ✅ 无需执行 |
| 参数化动态报表 v2.11 | 2026-04-15 | **无迁移脚本**。`GET /reports/{id}/data` 端点 + `report_params_service.py`（Jinja2 SQL 引擎）+ `_autoDetectFields` JS 函数 + `echarts_override.series` 系列模板修复均为纯代码层变更。使用已有 `reports.charts` JSONB（含 `sql`/`x_field`/`y_fields`/`connection_env`/`connection_type` 字段）和 `reports.filters` JSONB（含 `binds` 字段）；无新表、新列、新权限记录。 | ✅ 无需执行 |
| 动态报表 "Failed to fetch" 根因修复 v2.11.1 | 2026-04-16 | **无迁移脚本**。纯代码层 Bug 修复：A（`report_builder_service.py` + `report_service.py`：API_BASE 改为 IIFE+window.location.origin，`_api_base_url()` 无 PUBLIC_HOST 时返回 `""`）/ B（`reports.py:_get_or_init_ch_client()`：自动剥离 `"clickhouse-"` 前缀）/ C（`report_params_service.py:_normalize_binds()`：binds list→dict 容错）/ D（`.claude/skills/project/clickhouse-analyst.md`：添加格式警告）。无新表、新列、新权限记录。 | ✅ 无需执行 |
| SQL→Excel 按日期分块导出 v2.13 | 2026-05-07 | **DB 迁移脚本**：`backend/scripts/migrate_export_chunked.py`。`export_jobs` 表新增 3 列（均 nullable，非破坏性）：① `export_mode VARCHAR(20) DEFAULT 'single'`（枚举 `single` / `date_chunked`）；② `chunk_config JSONB`（`{date_column?, date_start, date_end, chunk_days, mode}`）；③ `output_files JSONB`（子文件清单 `[{index, date_start, date_end, filename, file_path, file_size, rows, sheets, status, _depth}]`）。幂等（`ALTER TABLE ... ADD COLUMN IF NOT EXISTS`）；存量任务自动获得 `export_mode='single'` 默认值，无影响。 | ✅ 已执行 |
| 数据导出可靠性增强 v2.14 | 2026-05-11 | **无迁移脚本**。Task C/A/B/D 四项改进 + 两个 bug 修均为代码层 + JSONB 字段扩展：① `chunk_config` JSONB 新增 `min_subdivide_unit: Literal["day","hour","minute"] = "day"`（Task A 小时/分钟级再细分 opt-in）+ `cursor_column: Optional[str] = None`（Task B 键集分页 opt-in，标识符白名单校验）；② `output_files[i]` JSONB 内部新增 `_retry_count: int`（Task D 失败原位重试计数）；③ 均沿用 v2.13 已加入的 `chunk_config` / `output_files` JSONB 列，**无 ALTER TABLE**。无新表、新权限记录（沿用 `data:export`，无新菜单/端点）；默认值保持 v2.13 行为，存量任务完全兼容。`backend/services/export_clients/clickhouse.py` 同时修两个隐性 bug：① `is_transient_stream_error` 沿 `__cause__/__context__` 探测（原 RuntimeError 包装吞掉指纹）；② keyset cursor 单调检查从 `>` 改为 `==`（原字典序数字误判）。 | ✅ 无需执行 |
| cursor_column 校验放宽 v2.14.1 | 2026-05-11 | **无迁移脚本**。纯代码层校验放宽:`backend/services/data_export_chunker.py` 新增 `_CURSOR_IDENT_RE = ^[A-Za-z_一-鿿][A-Za-z0-9_ 一-鿿]*$`(允许空格/中文,起首字母/下划线/中文);`validate_chunk_config` 中 cursor_column 校验从 `_IDENT_RE`(严格 SQL 标识符)切换到 `_CURSOR_IDENT_RE` + 自动 `strip("`")` 反引号 + 首尾空白。`backend/services/export_clients/clickhouse.py:stream_batches_keyset` 二次校验同步 + SQL 拼接统一用反引号包裹 `` ORDER BY `{cursor_column}` `` / `` WHERE `{cursor_column}` > {cursor_val:String} ``;`col_names.index()` 用裸字符串查找(TSV header 列名不含反引号)。用户场景:SELECT 别名常含空格(如 `` `Call ID` ``)或中文(如 `订单_id`),v2.14 严格白名单会 400 拒掉,v2.14.1 原生支持。无新字段、无新表、无新权限记录;现有 `chunk_config.cursor_column` JSONB 字段值字符集放宽但向后兼容。新增 6 个单测(D24-D29)+ 2 个集成测试(G7-G8),总测试数 340 全过零回归。 | ✅ 无需执行 |
| NULL cursor 错误信息升级 v2.14.2 | 2026-05-12 | **无迁移脚本**。纯代码层错误信息改进:`stream_batches_keyset` 当 cursor 列末行为 NULL 时,错误信息从模糊「cursor 列值为 None」升级为含 4 个可执行修复方案的可读建议:① SQL 加 `IS NOT NULL`;② 换 NOT NULL 列;③ 清空 cursor_column 回退 LIMIT/OFFSET;④ 先 count NULL 行数评估。新增 test_g9 验证错误信息含可执行关键词。无字段/表/权限变更。 | ✅ 无需执行 |
| Excel 引擎切换 openpyxl→xlsxwriter v2.14.3 | 2026-05-12 | **无迁移脚本**。纯代码层引擎切换:`_run_single_export` 把 `openpyxl.Workbook(write_only=True)` 替换为 `xlsxwriter.Workbook(path, options={"constant_memory": True, "strings_to_numbers": False, "strings_to_formulas": False, "strings_to_urls": False})`;`ws.append(row)` → `ws.write_row(cur_row, 0, row)` 显式维护行索引;cancel/fallback 路径 `wb.save()` 改为 `wb.close()`(xlsxwriter 在 close 时封 zip 落盘);attempt=1 fallback 切换前 `os.unlink(output_path)` 删除部分写入文件再重头来。性能:典型 26 列千万行场景写入速度 3-5x 提升(C 加速字符串处理 + 流式 zip);内存峰值与 batch_size 无关。**输出格式 100% 兼容**:大整数字符串保留为文本(无科学计数法)、公式样字符串(如 `=A1+B1`)不被识别为公式、跨库 xlsxwriter 写 + openpyxl 读 roundtrip 测试通过。无字段/表/权限变更;341/341 全量回归零失败。 | ✅ 无需执行 |
| ts 半开区间占位符 v2.14.4 | 2026-05-12 | **无迁移脚本**。纯代码层占位符语义扩展:`backend/services/data_export_chunker.py` 新增 `{{ts_start}}` / `{{ts_end}}` 一对占位符常量 + `has_ts_placeholders()` / `has_any_placeholders()` 检测函数 + `_format_ts_inclusive_start(v)` / `_format_ts_exclusive_end(v)` 字面量计算 helper(date 端点 → 'YYYY-MM-DD 00:00:00' / '(DD+1) 00:00:00';datetime 端点 → 原样 / +1s);`inject_date_filter()` 在占位符模式分支同时识别 ts 与 date 两套占位符,**两套可共存,各自替换各自**;`validate_chunk_config()` 把 `has_any_placeholders` 视同 placeholder 模式免 date_column 要求;`has_partial_placeholders` 扩展覆盖 ts 单占位符拒绝。**ts 占位符语义**:始终输出 `'YYYY-MM-DD HH:MM:SS'` 字面量 + 半开区间(`ts_end` 等于「下一窗口起点」),用户 SQL 统一写法 `WHERE col >= parseDateTimeBestEffort('{{ts_start}}') AND col < parseDateTimeBestEffort('{{ts_end}}')`,**day 模式与 sub-day 子块拆分完美兼容,无重叠无遗漏,DateTime64 亚秒精度安全**。解决场景:用户原 SQL `addDays(toDate('{{date_end}}'), 1)` 在 sub-day 下被 toDate 截断 → 整天数据重复 2 倍;用 `+ INTERVAL 1 SECOND` 在 sub-day 12h+12h 实际取 36 小时,严重重叠。ts 占位符两个问题都不存在。经典 date 占位符**保留不变,完全向后兼容**;`chunk_config` JSONB 字段无变化。新增 V1-V8(chunker)16 测试 + KN3(集成 sub-day 衔接)1 测试,357/357 全量回归零失败。 | ✅ 无需执行 |
