...
>
> **2026-04-09 迁移说明**：`clickhouse-analyst.md` 及其 8 个子 Skill 已正式迁移至 `project/` 层，`user/superadmin/` 下已清空这些文件。对话可见范围不变，写入保护升级为 Admin Token。
>
...
| 依赖的 sub_skills | — |
| 被哪些 Skill 引用 | — |
| 业务用途 | **分析启动协议**（v2.0）：Step1 双路径查知识库（优先用户私有→共享项目库），Step2 按需读表文档，Step3 确认目标环境，Step4 执行 SQL |
...
| 依赖的 sub_skills | — |
| 被哪些 Skill 引用 | — |
| 业务用途 | **知识库维护工作流**（v2.0）：支持共享库模式（写入 `_shared/db_knowledge/`）和用户私有库模式（写入 `{CURRENT_USER}/db_knowledge/`）；Step0 确认参数含写入目标选择 |

#### `clickhouse-analyst.md` ⭐ 父 Skill（从 user/superadmin/ 迁移，v2.0）

| 字段 | 值 |
|------|-----|
| 文件路径 | `.claude/skills/project/clickhouse-analyst.md` |
| tier / layer / scope | project / workflow / — |
| always_inject | false |
...
| 被哪些 Skill 引用 | — |
| 业务用途 | **外呼业务数据分析顶层工作流**：环境映射、核心表速查、计费规则知识、5个标准 SQL 模板、报告规范 |

#### 环境特化子 Skill（6 个，已迁移至 project）

| Skill 名 | env_tags | 文件路径 |
|---------|---------|---------|
| `ch-sg-specific` | `[sg]` | `project/ch-sg-specific.md` |
| `ch-idn-specific` | `[idn]` | `project/ch-idn-specific.md` |
| `ch-br-specific` | `[br]` | `project/ch-br-specific.md` |
| `ch-my-specific` | `[my]` | `project/ch-my-specific.md` |
| `ch-thai-specific` | `[thai]` | `project/ch-thai-specific.md` |
| `ch-mx-specific` | `[mx]` | `project/ch-mx-specific.md` |

#### 场景子 Skill（2 个，已迁移至 project）

| Skill 名 | scope | 文件路径 | 知识库引用 |
|---------|-------|---------|-----------|
| `ch-call-metrics` | global-ch | `project/ch-call-metrics.md` | `{SHARED_DATA_ROOT}/db_knowledge/metrics/` |
| `ch-billing-analysis` | global-ch | `project/ch-billing-analysis.md` | `{SHARED_DATA_ROOT}/db_knowledge/metrics/` |
...
### 2.3 Tier 3 用户层（`user/{username}/`）

> **2026-04-09 迁移说明**：原 `user/superadmin/` 下的 `clickhouse-analyst.md` 及 8 个子 Skill 已全部迁移至 `project/` 层（见 §2.2）。`user/superadmin/` 目前仅保留测试 Skill。

#### 当前用户层 Skill

| Skill 名 | 路径 | 说明 |
|---------|------|------|
| `evilskill.md` | `user/superadmin/evilskill.md` | 测试用 Skill（`test` 触发词），可忽略 |

#### 用户层使用指引

- 用户可通过 Web 页面或对话自由创建/编辑 `user/{username}/` 下的 Skill
- 用户探索的 db_knowledge 写入 `customer_data/{CURRENT_USER}/db_knowledge/`
- 用户 Skill 中引用知识库时，使用 `{CURRENT_USER}/db_knowledge/` 路径
- 若需要供所有人使用，可申请晋升到 project 层（晋升流程见 §2.4 知识库升级协议）
...
项目层
  clickhouse-analyst ────┬──► ch-sg-specific     (env: sg)
       （父 Skill）       ├──► ch-idn-specific    (env: idn)
                         ...
                         ├──► ch-call-metrics    ──► _shared/db_knowledge/metrics/
                         └──► ch-billing-analysis ──► _shared/db_knowledge/metrics/

  db-knowledge-router ── 独立触发（分析/查询/数据），查找顺序：
                         ① {CURRENT_USER}/db_knowledge/  (用户私有库优先)
                         ② _shared/db_knowledge/          (共享项目库)
  db-maintainer ──────── 独立触发（更新知识库），
                         支持两种写入目标：
                         - 用户库 {CURRENT_USER}/db_knowledge/
                         - 共享库 _shared/db_knowledge/ (管理员/Claude Code)
```
...
```
customer_data/
├── _shared/db_knowledge/      ← 共享项目知识库（管理员/Claude Code 维护）
│   ├── _index.md
│   ├── relationships.md
│   ├── tables/                ← 表文档（24 张，来自 SG 环境初始探索）
│   └── metrics/               ← 指标口径文档（5 个）
└── {username}/db_knowledge/   ← 用户私有知识库（个人探索/覆盖）
    └── （结构相同，按需创建）
```
...
### 2.5 Skill 晋升协议（user → project）

当用户 Skill 稳定且有共享价值，可申请晋升至 project 层：

**晋升步骤（管理员/Claude Code 执行）：**

1. **路径替换**：将 Skill 内容中的 `{CURRENT_USER}/db_knowledge/` 替换为 `{SHARED_DATA_ROOT}/db_knowledge/`
2. **知识库迁移**：将用户私有知识库中被引用的文件合并至 `_shared/db_knowledge/`（不覆盖已有更新内容）
3. **写入 project 层**：通过 Admin Token 调用 `POST /api/v1/skills/project-skills/`，或直接写入 `.claude/skills/project/{name}.md`
4. **从 user 层删除**（可选）：若原 Skill 已被 project 层替代，从 `user/{username}/` 删除，避免重复触发

**Claude Code 执行晋升命令参考：**
```
1. 复制文件：cp .claude/skills/user/{username}/{skill}.md .claude/skills/project/{skill}.md
2. 替换路径：sed -i 's|{CURRENT_USER}/db_knowledge/|{SHARED_DATA_ROOT}/db_knowledge/|g' .claude/skills/project/{skill}.md
3. 合并知识库：cp -n customer_data/{username}/db_knowledge/... customer_data/_shared/db_knowledge/...
4. 版本递增：在 frontmatter 更新 version 字段
```
...
**核心分工（2026-04-09 更新）**：
- **Claude Code CLI** = 维护 `project/` 层 Skill 和共享知识库 `_shared/db_knowledge/`
- **项目页面对话** = 用户探索数据，写入 `user/{username}/db_knowledge/`（个人私有库）

**决策矩阵：**

| 场景 | Claude Code CLI | 项目页面（用户对话） | 推荐 |
|------|----------------|-------------------|------|
| 维护共享知识库（所有人可用）| ✅ 直接读写 `_shared/db_knowledge/`，Git 跟踪 | ⚠️ 需说"更新共享知识库"，无 Git | **CLI** |
| 探索新业务表 → 写用户私有库 | ❌ 无 ClickHouse 连接 | ✅ Agent 查询 → 写 `{CURRENT_USER}/db_knowledge/` | **页面** |
| 优化/维护 project 层 Skill 措辞 | ✅ 多文件编辑，可 diff 审查，Git 提交 | ⚠️ 可写但无 Git 跟踪 | **CLI** |
| 用户创建自己的 Skill | ❌ 路径隔离（只能写自己目录） | ✅ 对话中自然创建 | **页面** |
| Skill 晋升 user → project | ✅ 执行晋升协议（§2.5），路径替换+知识库合并 | ❌ 无 Admin Token | **CLI** |
| 生产环境紧急安全规则调整 | ✅ 编辑 system/ → Git 提交 + CR | ⚠️ 修改即生效，无审查 | **CLI** |
| 调试 Skill 触发词 | ⚠️ 需在页面验证 | ✅ /skills 页面触发测试面板 | **页面** |

**推荐工作流组合：**

```
1. 在 Web 界面探索新业务表 → 写入用户私有库 {CURRENT_USER}/db_knowledge/
   说"更新 SG 的知识库" → db-maintainer 自动执行（写用户库）

2. 确认内容正确后，在 Claude Code CLI 将知识合并到共享库：
   cp -n customer_data/{username}/db_knowledge/... customer_data/_shared/db_knowledge/...

3. Skill 内容优化 → Claude Code CLI（读文件 → 修改 → Git 提交）

4. 若用户 Skill 有共享价值 → CLI 执行 §2.5 晋升协议

5. 触发词调试 → 项目页面 /skills 触发测试面板

6. 紧急安全规则 → CLI 编辑 system/ → Git 提交 → 部署
```
...
│
├── project/
│   ├── db-knowledge-router.md   [triggered: 分析/查询/数据...] ← 分析启动协议（双路径查找）
│   ├── db-maintainer.md         [triggered: 更新知识库...]    ← 维护工作流（支持共享库模式）
...
```
customer_data/
├── _shared/db_knowledge/        ← 共享项目知识库（Claude Code 维护，所有用户可读）
│   ├── _index.md                  全局索引
│   ├── relationships.md           ERD + 业务链路图
│   ├── tables/                    24 张表文档（SG 环境初始探索）
│   │   ├── realtime_dwd_crm_call_record.md
│   │   └── ...
│   └── metrics/                   5 个指标口径文档
│       ├── connect_rate.md
│       └── ...
│
└── {username}/db_knowledge/     ← 用户私有知识库（个人探索/覆盖，优先于共享库）
    └── （结构相同，按需创建）
```
